...
export function GasKpiCards({ data }: GasKpiCardsProps) {
  ...
  const cards = [
    ...
    {
      title: "緊急ワード検知",
      value: stats.emergencyKeywordCount.toLocaleString(),
      unit: "件",
      change: "今月の検知総数",
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
      {cards.map((card) => (
        <Card
          key={card.title}
          className={`border-border/60 ${card.highlight ? "bg-destructive/5" : ""}`}
        >
            ...
            <div className="flex items-start justify-between">
              ...
              <div
                className={`rounded-lg p-2.5 ${
                  card.highlight ? "bg-destructive/10" : "bg-primary/10"
                }`}
              >
                <card.icon
                  className={`h-5 w-5 ${
                    card.highlight ? "text-destructive" : "text-primary"
                  }`}
                />
              </div>
            </div>
            <div className="mt-3 flex items-center gap-1.5">
              <span
                className={`text-xs font-medium ${
                  card.changePositive ? "text-accent" : "text-muted-foreground"
                }`}
              >
                {card.change}
              </span>
